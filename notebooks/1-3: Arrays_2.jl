...
The slice operators are `b:e` and `b:s:e`, where `b`, `s`, and `e` are the beginning, step, and ending values. This step (or three value) variation differs slightly from the Python syntax of `b:e:s`.
...
```
!!! note
	The slice operator calculates the begin and end values, but does not generate an array. It just returns itself. This behavior means that a computation is only performed when needed.
"""
...
!!! note
	Arrays allocate memory, whereas tuples do not. Therefore, tuples have better performance than arrays, but are limited to a single dimension and lengths of less than about 1000 elements.
"""
...
What have we learned about array comprehension and slicing.
